...
$this->registerJs(
	...
	.'$(window).bind("hashchange",  function(){'
		...
		.'} else if (location.hash.includes("#dialogs")){'
			...
			.'})'
	  			.'.done(function(dialogs) {'
					.'drawDialogs(dialogs);'
					.'timerId = setInterval(checkDialogs, 10000);'
	  			.'});'
		.'} else if (location.hash.includes("#messages")){'
			...
			.'})'
	  			.'.done(function(result) {'
					.'drawMessages(result);'
					.'timerId = setInterval(checkMessages, 10000);'
	  			.'});'
		.'} else if (location.hash == "#proposals"){'
			...
			.'})'
	  			...
	  			.'});'
		.'} else if (location.hash.includes("#proposal")){'
			.'activateLink(2);'
			.'var data = {};'
			.'var query = location.hash.split("?");'
			.'if (query.length==2){'
			...
			.'$.ajax({'
	  			.'method: "GET",'
	  			.'url: "'.Url::toRoute(['order/proposal']).'",'
	  			.'data: data'
			.'})'
	  			.'.done(function(result) {'
					.'drawProposal(result);'
	...
	.'var drawDialogs = function(dialogs){'
		...
		.'if (dialogs != "" && dialogs.length > 0){'
			.'html += "<ul class=\"dialogs mdl-list mdl-shadow--2dp\">";'
			.'dialogs.forEach(function(dialog) {'
				.'html += _drawDialog(dialog);'
			.'});'
			.'html += "</ul>";'
		.'} else {'
			.'html += "<div class=\"section-title mdl-typography--display-1-color-contrast\">Нет личных сообщений</div>'
					.'<p class=\"section--center mdl-grid mdl-color-text--grey-500\">Здесь вы будете видеть и отвечать на сообщения от Мастеров, заинтересованных в проекте.</p>";'
		.'}'
		.'content.append(html);'
	.'};'
	.'var _drawDialog = function(dialog){'
		.'return "<a dialog=\""+dialog["id"]+"\" id=\""+dialog["interlocutorId"]+"\" href=\"#messages?'.(Yii::$app->user->identity->master?'userId="+dialog["userId"]+"':'masterId="+dialog["masterId"]+"').'\" class=\"mdl-list__item mdl-list__item--two-line\">'
  				...
  				.'</a>";'
	.'};'
	.'var checkDialogs = function(){'
		.'var item = $(".dialogs.mdl-list>.mdl-list__item:first");'
		.'if (item != null){'
			.'var last = item.attr("dialog");'
			.'var data = {};'
			.'data["id"] = '.$order->id.';'
			.'data["last"] = last;'
			.'$.ajax({'
			...
			.'})'
	  			.'.done(function(dialogs) {'
					.'console.log("dialogs");'
					.'if (dialogs != null && dialogs != ""){'
						.'for(var i = dialogs.length - 1; i >= 0; i--) {'
							.'var dialog = dialogs[i];'
							.'var widget = $("#"+dialog["interlocutorId"]);'
							.'if (widget != null){'
								.'widget.find(".mdl-list__item-sub-title").html((dialog["author"]=='.Yii::$app->user->getId().'?"Вы: ":"")+dialog["text"]);'
								.'widget.find(".mdl-list__item-secondary-info").html((dialog["dateStr"]=="сегодня"?dialog["timeStr"]:dialog["dateStr"]));'
								.'widget.attr("dialog", dialog["id"]);'
								.'widget.parent().prepend(widget);'
							.'} else {'
								.'var list = $(".dialogs.mdl-list");'
								.'list.prepend(drawDialog(dialog));'
							.'}'
						.'}'
					.'}'
	  			.'});'
	...
	.'var drawMessages = function(result){'
		...
		.'var content = $("#tab-content");'
		.'content.empty();'
		.'var html = "<div class=\"messages mdl-card mdl-shadow--2dp\">'
		...
		.'messages.forEach(function(message) {'
			...
			.'}'
			.'html += _drawMessage(message, interlocutor);'
		.'});'
		.'html += "</ul></div>";'
		.'html += "<div class=\"message-form mdl-card__actions mdl-card--border\">'
					.'<form id=\"messageform\" action=\"'.Url::toRoute(['order/message', 'id' => $order->id]).'\" method=\"post\">'
						.'<div class=\"mdl-grid mdl-card__subtitle-text\">'
							...
							.'</div>'
							.'<input style=\"display:none\" name=\"MessageForm[interlocutor]\" id=\"messageform-interlocutor\" value=\""+interlocutor["id"]+"\">'
		...
		.'$("#action_send").click(function(){'
			...
			.'return false;'
		.'});'
		.'$("#messageform-message").keydown(function(event){'
		...
		.'});'
	.'};'
	.'var _drawMessage = function(message, interlocutor){'
		.'var name;'
		...
		.'}'
		.'return "<div class=\"mdl-list__item mdl-list__item--many-line\" id=\"message_"+message["id"]+"\">'
		...
		.'if (message != null && message.trim() != ""){'
			...
			.'}).success(function(response) {'
				.'if (response != null && response != ""){'
					...
					.'var list = $(".messages.mdl-list");'
					.'var lastDate = list.find(".message-date:last span");'
					.'if (lastDate !== null && lastDate.html() !== response["dateStr"]){'
						.'list.append("<h5 class=\"message-date\"><span>"+response["dateStr"]+"</span></h5>");'
					.'}'
					.'list.append(drawMessage(response, null));'
					.'$(".message-body").scrollTop($(".message-body")[0].scrollHeight);'
				.'}'
	...
	.'var checkMessages = function(){'
		.'var item = $(".messages.mdl-list>.mdl-list__item:last");'
		.'if (item != null){'
			.'var last = item.attr("id").split("_")[1];'
			.'var data = {};'
			.'var query = location.hash.split("?");'
			.'if (query.length==2){'
				.'data = getParams(query[1]);'
	  			...
	  			.'.done(function(result) {'
					.'if (result != null && result["messages"] != null && result["messages"].length > 0){'
						...
						.'result["messages"].forEach(function(message) {'
							.'if (date !== message["dateStr"]){'
								.'list.append("<h5 class=\"message-date\"><span>"+message["dateStr"]+"</span></h5>");'
								.'date = message["dateStr"];'
							.'}'
							.'list.append(drawMessage(message, result["interlocutor"]));'
						.'});'
						.'$(".message-body").scrollTop($(".message-body")[0].scrollHeight);'
					.'}'
	  			.'});'
	...
	.'var drawProposals = function(proposals){'
		...
		.'if (proposals != "" && proposals.length > 0){'
			.'html += "<div class=\"proposals-card mdl-card mdl-shadow--2dp\">'
							.'<table class=\"md-table md-row-click\">'
								.'<thead class=\"md-head\">'
									.'<tr class=\"md-row\">'
										.'<th class=\"md-column\">Мастер</th>'
										.'<th class=\"md-column\">Условия оплаты</th>'
										.'<th class=\"md-column\">Время изготовления</th>'
										.'<th class=\"md-column md-numeric\">Цена</th>'
		              				.'</tr>'
		            			.'</thead>'
		            			.'<tbody class=\"md-body\"></tbody>'
							.'</table>'
						.'</div>";'
			.'content.append(html);'
			.'var table = content.find(".proposals-card .md-table .md-body");'
			.'proposals.forEach(function(proposal) {'
				.'table.append(_drawProposal(proposal));'
			.'});'
		.'} else {'
			.'html += "<div class=\"section-title mdl-typography--display-1-color-contrast\">Нет предложений</div>'
					.'<p class=\"section--center mdl-grid mdl-color-text--grey-500\">Здесь вы будете видеть и отвечать на официальные предложения со стороны Мастеров, готовых к работе над вашим проектом.</p>";'
			.'content.append(html);'
		.'}'
	.'};'
	.'var _drawProposal = function(proposal){'
		.'var html = "<tr id=\""+proposal["id"]+"\" class=\"md-row\">";'
		.'html += "<td class=\"md-cell\">";'
		.'html += proposal["firstName"]+" "+proposal["lastName"];'
		.'html += "</td>";'
		.'var schedules = "";'
		.'if (proposal["schedules"] == 0){'
			.'schedules = "100% по завершению";'
		.'} else if (proposal["schedules"] == 100){'
			.'schedules = "100% предоплата";'
		.'} else {'
			.'schedules = proposal["schedules"]+"% аванс / "+(100-proposal["schedules"])+"% по завершению";'
		.'}'
		.'html += "<td class=\"md-cell\">"+schedules+"</td>";'
		.'html += "<td class=\"md-cell\">"+dayesToString(proposal["estimated"])+"</td>";'
		.'html += "<td class=\"md-cell md-numeric\">"+proposal["price"]+" руб."+"</td>";'
		.'html += "</tr>";'
		.'var result = $(html);'
		.'result.click(function(){'
			.'location.hash = "#proposal?masterId="+proposal["masterId"];'
		.'});'
		.'return result;'
	.'};'
	.'var drawProposal = function(proposal){'
		.'var content = $("#tab-content");'
		.'content.empty();'
		.'var schedules = "";'
		.'var terms = "";'
		.'if (proposal["schedules"] == 0){'
			.'schedules = "100% по завершению";'
			.'terms = proposal["price"] + " руб. когда ваш проект будет завершен";'
		.'} else if (proposal["schedules"] == 100){'
			.'schedules = "100% предоплата";'
			.'terms = proposal["price"] + " руб. когда вы примите это предложение";'
		.'} else {'
			.'schedules = proposal["schedules"]+"% аванс / "+(100-proposal["schedules"])+"% по завершению";'
			.'terms = proposal["price"]*proposal["schedules"]/100 + " руб. когда вы примите это предложение, " + proposal["price"]*(100-proposal["schedules"])/100 + " руб. когда ваш проект будет завершен";'
		.'}'
		.'var html = "<div class=\"proposal-card mdl-card mdl-shadow--2dp\">'
					.'<div class=\"mdl-card__header mdl-color--primary\">'
    					.'<span class=\"mdl-card__header-text\">"+proposal["price"]+" руб."+"</span>'
						.'<a href=\"#proposals\" class=\"mdl-button mdl-js-button mdl-button--icon mdl-card__header-button\">'
    						.'<i class=\"material-icons\">arrow_back</i>'
						.'</a>'
  					.'</div>'
					.'<div class=\"mdl-card__actions\">'
						.'<div class=\"mdl-grid mdl-card__subtitle-text\">'
							.'<div class=\"mdl-cell mdl-cell--12-col master-content\">'
								.'<a class=\"avatar\" style=\"background-image: url(\'"+proposal["avatar"]+"\');\"></a>'
								.'<div class=\"details\">'
									.'"+proposal["firstName"]+" "+proposal["firstName"]+"<br>'
									.'"+proposal["shop"]+" / "+proposal["placeName"]+"<br>'
									.str_replace('"', '\"', StarsRating::widget([
											'rating' => 4,
											'reviews' => 10,
											'isSmall' => true,
					...
					.'<div class=\"mdl-card__actions mdl-card--border\">'
						.'<div class=\"mdl-grid mdl-card__subtitle-text\">'
							.'<div class=\"mdl-cell mdl-cell--3-col\">Ожидаемое время доставки</div>'
							.'<div class=\"mdl-cell mdl-cell--9-col\">"+dayesToString(proposal["estimated"])+"</div>'
						.'</div>'
						.'<div class=\"mdl-grid mdl-card__subtitle-text\">'
							.'<div class=\"mdl-cell mdl-cell--3-col\">Условия оплаты</div>'
							.'<div class=\"mdl-cell mdl-cell--9-col\">"+schedules+"</div>'
						.'</div>'
						.'<div class=\"mdl-grid mdl-card__subtitle-text\">'
							.'<div class=\"mdl-cell mdl-cell--3-col\">График оплаты</div>'
							.'<div class=\"mdl-cell mdl-cell--9-col\">"+terms+"</div>'
						.'</div>'
						.'<div class=\"mdl-grid mdl-card__subtitle-text\">'
							.'<div class=\"mdl-cell mdl-cell--3-col\">Цена</div>'
							.'<div class=\"mdl-cell mdl-cell--9-col\">"+proposal["price"]+" руб."+"</div>'
						.'</div>'
					.'</div>'
					.'<div class=\"mdl-card__actions mdl-card--border\">'
						.'<a id=\"agree_button\" class=\"mdl-button mdl-button--primary mdl-js-button mdl-js-ripple-effect\">'
      						.'Принять'
    					.'</a>'
						.'<a href=\"#messages?masterId="+proposal["masterId"]+"\" class=\"mdl-button mdl-button--accent mdl-js-button mdl-js-ripple-effect\">'
      						.'Написать'
    					.'</a>'
  					.'</div>'
				.'</div>'
				.'<dialog id=\"agree_dialog\" class=\"confirm-dialog mdl-dialog\">'
					.'<div class=\"mdl-dialog__content\"><p>Вы действительно хотите принять это предложение?</p></div>'
					.'<div class=\"mdl-dialog__action\">'
						.'<button id=\"dialog_yes\" type=\"button\" class=\"mdl-button\">Да</button>'
						.'<button id=\"dialog_no\" type=\"button\" class=\"mdl-button close\">Нет</button>'
					.'</div>'
			...
			.'";'
		.'content.append(html);'
		.'var agreeDialog = content.find("#agree_dialog");'
		.'if (!agreeDialog[0].showModal) {'
			.'dialogPolyfill.registerDialog(agreeDialog[0]);'
		.'}'
		.'agreeDialog.find("#dialog_yes").click(function(){'
			.'agreeDialog[0].close();'
			.'agreeProposal(proposal["id"]);'
		.'});'
		.'agreeDialog.find("#dialog_no").click(function(){'
			.'agreeDialog[0].close();'
		.'});'
		.'content.find("#agree_button").click(function(){'
			.'agreeDialog[0].showModal();'
		.'});'
	.'};'
	.'var agreeDialog = function(id){'
		.'$.ajax({'
  			.'method: "GET",'
  			.'url: "'.Url::toRoute(['proposal/agree']).'",'
  			.'data: {"id": id},'
  			.'statusCode: {'
				.'400: function() {'
					.''
